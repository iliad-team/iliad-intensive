...
\begin{itemize}
    ...
    \item How does internal structure (e.g. goals) form during training?
    \item How can we intervene on the data, training objective, architecture, and optimizer to shape which internal structures are learned throughout training?
\end{itemize}
\end{frame}
\begin{frame}{How: making mathematical models of learning}
    \begin{itemize}
        \item Tractable toy models: deep linear networks, shallow nonlinear networks, tensor networks, polynomial neural networks
        ...
        \item Identify useful observables: learning coefficient, correlation functions, sufficient statistics...
...
\begin{frame}{Safety motivations}
    \begin{itemize}
        \item A central question in AI safety is how can we shape the training process to select an aligned instead of a harmful model (scheming, spiteful...)
        \item Develop stronger interpretability methods by decomposing models throughout training
        \item Our approach is to develop better models of training to find control levers in the training process that make us confident training will select a safe model
    \end{itemize}
\end{frame}
...
\begin{frame}{Lazy regime (NTK or linear regime)}
  ...
  This behaves like a shallow linear network: student interpolates between $M$ and a random matrix $W_0$.
\end{frame}
\begin{frame}{Alignment happens at the same time as learning singular values}
  \begin{figure}
        \centering
        \includegraphics[width=\linewidth,height=0.75\textheight,keepaspectratio]{alignment_lazy.png}
    \end{figure}
\end{frame}
\begin{frame}{Rich regime}
  ...
  \]
\end{frame}
\begin{frame}{Alignment happens before learning singular values}
  \begin{figure}
        \centering
        \includegraphics[width=\linewidth,height=0.75\textheight,keepaspectratio]{alignment_rich.png}
    \end{figure}
\end{frame}
\begin{frame}{Rich vs Lazy}
  \begin{figure}
        \centering
        \includegraphics[width=\linewidth,height=0.75\textheight,keepaspectratio]{comparison_rich_vs_lazy.png}
    \end{figure}
\end{frame}
\begin{frame}{Takeaways}
  \begin{block}{Key points}
    \begin{itemize}
      \item Depth and specific initialization induce different learning regimes
      \item Large initialization and large width give the NTK regime: no feature learning, and the NTK is frozen: linear regression.
      \item In the saddle to saddle regime, learning happens in two phases: task alignment and amplification.
...
\begin{block}{Key points}
  \begin{itemize}
      \item Nonlinearity modeled with gated DLNs: neural race reduction
      \item MLPs and linear transformers also have saddle-to-saddle dynamics
      \item Alternating gradient flow on quadratic networks: towards a more general theory of the saddle-to-saddle regime
  \end{itemize}
\end{block}
\end{frame}
