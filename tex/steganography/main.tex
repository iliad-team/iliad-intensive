...
%   * summary: TODO -- no source has an overview paragraph.
%   * Notation collides across the two halves: lambda is both the security
%     parameter (2^lambda attack cost) and the blend coefficient (0<=lambda<=1);
%     m is both the hidden message and the number of ReLU measurements; p is
%     both the prompt and a sample fraction; sigma is both a code symbol and a
%     standard deviation. Fixing these means renotating, which the port mandate
%     forbids -- left for an editor with authoring rights.
...
\begin{enumerate}
  ...
  \item \textbf{Make the PRF depend on the hidden symbol}. To transmit a code
        symbol $\sigma \in \{0,1,\leftarrow\}$, the generator uses randomness
  ...
  \item \textbf{Decoder scores all candidate symbols.} The decoder knows $k$,
        ...
        symbol: $\sigma = 0,1,\leftarrow$ The correct symbol's score tends to
  ...
  \item Use dynamic error correction. Because the decoder can occasionally pick
        ...
        error-correcting code over $\{0,1,\leftarrow\}$. The symbol
...
\[
    x' = \sqrt{1-\lambda^2} x + \lambda s, \quad \text{where} \quad 0\leq\lambda\leq 1.
\]
...
\draw[widtharrow]
    ...
    node[midway, above] {$\frac{\sigma}{\sqrt{m}}$};
...
\node[red!70!black, left=3pt] at (\taupos,\tauheight) {$"0"$};
\node[red!70!black, right=3pt] at (\taupos,\tauheight) {$"1"$};
...
Gaussian measurements. As the number \(m\) of ReLU measurements grows, both
empirical averages concentrate around their means at scale \(1/\sqrt m\).
...
\(g \sim \mathcal N(0,I_d+\theta ss^\top)\), and \(x' = \sqrt{1-\lambda^2}x+\lambda s\).
Show that
\[
    z := \langle g,x\rangle \sim \mathcal N(0,1)
    \quad \text{and} \quad
    z' := \langle g,x'\rangle
    \sim \mathcal N\bigl(0,1+\lambda^2\theta\bigr).
\]
\textbf{Hint:} When $g \sim \mathcal N(0, \Sigma)$, then $\operatorname{Var}(\langle g,x\rangle) = x^\top \Sigma x $
...
sample whenever \(\lambda > 0\) and \(\theta > 0\).
...
\[
    ...
    \leq \sigma^2.
\]
For independent copies \(Z_1,\dots,Z_m\), and
\[
   A_m := \frac{1}{m}\sum_{i=1}^m \operatorname{ReLU}(Z_i),
\]
calculate
$ \mathbb E \left(
        A_m
...
\textbf{Bonus:} Assume that the regular pre-threshold activation \(A_m\) satisfies
\[
    \mathbb E[A_m] = \mu_{\text{reg}},
    \qquad
    \operatorname{Var}(A_m) \leq \frac{1}{m}.
\]
Suppose the threshold \(\tau\) is chosen such that a fixed fraction \(p>0\)
of regular samples are classified with label \(1\), i.e.
\[
    \Pr[A_m \geq \tau] \geq p.
\]
...
\[
    A'_m
    :=
    \frac{1}{m}\sum_{i=1}^m \operatorname{ReLU}(z'_i),
\]
where the \(z'_i\) are independent copies of
\[
    z' \sim \mathcal N\bigl(0,1+\lambda^2\theta\bigr).
\]
Let
$    \mu_{\mathrm{bd}}
    :=
    \mathbb E[A'_m]$.
Show that for large enough \(m\),
\[
    \Delta_m := \mu_{\mathrm{bd}}-\tau > 0.
\]
Use Chebyshev's inequality to show that
\[
    \Pr[A'_m < \tau]
    \leq
    \frac{1+\lambda^2\theta}{m\Delta_m^2}.
\]
Conclude that, whenever \(\Delta_m\) is bounded below by a positive constant,
the fraction of backdoored samples classified as \(0\) decays like \(1/m\).
...
\begin{enumerate}
\item Classify all normal samples as ``0'', making the classifier meaningless
\item Classify all backdoored samples as ``1''.
\end{enumerate}
...
Similarly, \(z'=\langle g,x+\lambda s\rangle\) is centered Gaussian with
variance
\[
\begin{aligned}
    \operatorname{Var}(z')
    &=
    \left(\sqrt{1-\lambda^2}x+\lambda s\right)^\top (I_d+\theta ss^\top)\left(\sqrt{1-\lambda^2}x+\lambda s\right) \\
    &=
    \|\sqrt{1-\lambda^2}x+\lambda s\|_2^2
    +
    \theta \left\langle s,\sqrt{1-\lambda^2}x+\lambda s\right\rangle^2.
\end{aligned}
\]
...
\[
    \|x+\lambda s\|_2^2=1,
    \qquad
    \left\langle s,\sqrt{1-\lambda^2}x+\lambda s\right\rangle=\lambda.
\]
Therefore
\[
    \operatorname{Var}(z')
...
\[
    \mathbb E[\operatorname{ReLU}(z')]
    =
    \frac{\sqrt{1+\lambda^2\theta}}{\sqrt{2\pi}}.
\]
Since \(\lambda>0\) and \(\theta>0\), we have
\[
    \sqrt{1+\lambda^2\theta} > 1,
\]
...
\begin{aligned}
    ...
    \operatorname{Var}
    \left(
        \frac{1}{m}\sum_{i=1}^m \operatorname{ReLU}(Z_i)
    \right) \\
    &=
    \frac{1}{m^2}
    \sum_{i=1}^m
...
\[
    \{A_m\geq \tau\}
    \subseteq
    \{|A_m-\mu_{\text{reg}}|\geq \tau-\mu_{\text{reg}}\}.
\]
...
\[
    ...
    \right]
    \leq
    \frac{\operatorname{Var}(A_m)}
    {(\tau-\mu_{\text{reg}})^2}.
\]
Using \(\operatorname{Var}(A_m)\leq 1/m\), we get
\[
    p
    \leq
    \frac{1}{m(\tau-\mu_{\text{reg}})^2}.
\]
Rearranging gives
\[
    \tau-\mu_{\text{reg}}
...
\[
    ...
    =
    \frac{\sqrt{1+\lambda^2\theta}-1}{\sqrt{2\pi}}
    >
    0.
\]
Using part (d),
...
\[
    ...
    \frac{1}{\sqrt{mp}}.
\]
For large enough \(m\), the last term is smaller than
\(\mu_{\mathrm{bd}}-\mu_{\mathrm{reg}}\), so
\[
    \Delta_m>0.
\]

Now apply Chebyshev's inequality to \(A'_m\). Since
\[
    \mathbb E[A'_m]=\mu_{\mathrm{bd}},
\]
we have
\[
\begin{aligned}
    \Pr[A'_m<\tau]
    &=
    \Pr[\mu_{\mathrm{bd}}-A'_m>\mu_{\mathrm{bd}}-\tau] \\
    &\leq
    \Pr[|A'_m-\mu_{\mathrm{bd}}|\geq \Delta_m] \\
    &\leq
    \frac{\operatorname{Var}(A'_m)}{\Delta_m^2}.
\end{aligned}
\]
By part (c), with
\[
    \sigma^2=1+\lambda^2 \theta,
\]
we have
\[
    \operatorname{Var}(A'_m)
    \leq
    \frac{1+\lambda^2\theta}{m}.
\]
Thus
\[
    \Pr[A'_m < \tau]
    ...
    \frac{1+\lambda^2\theta}{m\Delta_m^2}.
\]
If \(\Delta_m\) is bounded below by a positive constant, then the right-hand
side is \(O(1/m)\). Hence the fraction of backdoored samples classified as
\(0\) decays like \(1/m\).
