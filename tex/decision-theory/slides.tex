...
% Preserve all source content while making the dense lecture slides readable:
% use the wider teaching format, reclaim unused chrome, and raise the two
% smallest explicit type sizes by one step.
\setbeamersize{text margin left=9mm,text margin right=9mm}
\setbeamertemplate{navigation symbols}{}

\let\iliadoriginalscriptsize\scriptsize
\let\iliadoriginalfootnotesize\footnotesize
\renewcommand{\tiny}{\iliadoriginalscriptsize}
\renewcommand{\scriptsize}{\iliadoriginalfootnotesize}

\defbeamertemplate*{headline}{infolines theme}
{
	\begin{beamercolorbox}[wd=\paperwidth,ht=3mm,right]{white}%
...
\setbeamertemplate{footline}
{
	\leavevmode
	\hbox{%
	\begin{beamercolorbox}[wd=0.9\paperwidth,ht=2.5ex,dp=1ex,leftskip=9mm]{normal text}%
		\usebeamerfont{footline}\insertsection
	\end{beamercolorbox}%
	\begin{beamercolorbox}[wd=0.1\paperwidth,ht=2.5ex,dp=1ex,rightskip=9mm plus 1fil]{normal text}%
		\usebeamerfont{footline}\insertpagenumber
	\end{beamercolorbox}%
	}
}
...
\frame[shrink=8]{\frametitle{Causal decision theory: seeing vs.\ doing}
	\begin{columns}[T]
...
\frame[shrink=2]{\frametitle{Joint prediction $=$ EDT}
...
\frame[shrink=5]{\frametitle{Counterfactual mugging: the updateless move}
	\begin{columns}[T]
	\begin{column}{0.58\textwidth}
...
\frame[shrink=4]{\frametitle{UDT 1.1: optimise the whole policy, not one action}
	\begin{columns}[T]
...
\frame[shrink=5]{\frametitle{The 5-and-10 problem}
	\begin{columns}[T]
