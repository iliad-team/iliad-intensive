...
\emph{degeneracy} in neural networks. What exactly is degeneracy? Let us first
appeal to the Cambridge dictionary, which documents some features of typical
uses of the word within mathematics:
\begin{quote}
...
\end{quote}
This dictionary sense already captures the intuition we will make precise:
a degenerate object sits at the boundary of a family, where something that is
usually nonzero happens to vanish.
...
\subsection{Neural networks, parameters, and parameter--function maps}
...
\begin{remark}[Parameters]
    ...
    \begin{enumerate}
        \item
            ...
            neural network has billions of parameters'').
        \item
...
\begin{enumerate}
    ...
    \item
        The \textbf{rectified linear unit (ReLU)} function $\relu(z) = \max(z, 0)$.
\end{enumerate}
...
\label{ex:biased-neurons}
    ...
    biased neurons throughout. Define precisely the parameter space and the
    parameter--function map in each case, and count the parameters.
\end{exercise}
...
\begin{proof}
    ...
    Fix $t_0 \in \R$. This is the only step where continuity of the symmetry
    is used. Since $T_t$ is a continuous symmetry, the derivative
    along the curve at $t_0$ vanishes:
    \begin{align*}
        D_{\gamma'(t_0)} \Phi(\gamma(t_0))
        &\propto \gamma'(t_0) \cdot \nabla \Phi(\gamma(t_0))
    \\  &= \left.\frac{d}{dt}\right|_{t=t_0} \Phi(\gamma(t))
            \tag{chain rule for $\Phi \circ \gamma$}
...
\label{thm:fef}
    ...
    \label{eq:fef}
        F_n(\U) = nL_n(w^*) + \lambda_{\U} \log n 
              - (\mu_{\U} - 1)\log \log n + O_p(1), \qquad n \to \infty.
    \end{equation}
\end{theorem}
