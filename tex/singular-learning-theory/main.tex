...
\usepackage[pdftex,colorlinks,allcolors=blue!60!black,backref=page]{hyperref}
\usepackage{amsmath}
...
% Fancy backreferences in bibliography https://tex.stackexchange.com/a/383771
% (note: must also pass option 'backref=page' to hyperref, and load natbib
\renewcommand*{\backref}[1]{}
\renewcommand*{\backrefalt}[4]{%
    \ifcase #1\or Cited on page~#2.\else Cited on pages~#2.\fi%
}
...
\declaretheorem[name=Corollary,numberlike=theorem,style=plain]{corollary}
...
\declaretheorem[name=Example,numberlike=theorem,style=definition]{example}
...
\renewcommand{\theHexthm}{\thesection.\arabic{exthm}}
\newtcolorbox{exbox}{
    oversize,
    breakable,
    size=minimal,boxsep=3mm,arc=3mm,
    enhanced jigsaw,
    frame hidden,
}
\newenvironment{exercise}[1][]{%
    \begin{exbox}%
    \begin{exthm}[#1]%
    \crefalias{enumi}{exthm}%
    \crefalias{enumii}{exthm}%
    \setlist[enumerate,1]{label={(\alph*)},ref={\theexthm(\alph*)}}%
...
% Solutions
% Usage: \begin{solution}{ex:label-of-exercise} ... \end{solution}
% The mandatory argument is the label of the corresponding exercise.
% The solution heading will include a backlink to the exercise.
% Unnumbered: the exercise reference in the heading identifies the solution.
% The wrapper adds a label sol:<exercise-label> for \listoftheorems linking.
\declaretheoremstyle[
    notebraces={}{},
    qed=\qedsymbol,
]{solutionstyle}
\declaretheorem[name=Solution,numbered=no,style=solutionstyle]{solthm}
\newenvironment{solution}[1]{%
    \begin{solthm}[to \cref{#1} (\nameref{#1})]%
    \label{sol:#1}%
    \leavevmode%
...
\newcommand\authorname[1]{\makebox[0.285\textwidth]{\normalsize\textbf{#1}}}
\newcommand\affiliation[1]{\makebox[0pt]{\normalsize#1}}
...
surveying the literature on SLT and deep learning. \Cref{apx:solutions}
...
\begin{enumerate}
    \item
        ...
        \hint{use the limit definition, \eqref{eq:directional-derivative}.}
    \item
...
\begin{enumerate}
    ...
    \item
        ...
        such that $D_v L(w) = 0$. \hint{consider separately the cases $\nabla
        L(w) = 0$ and $\nabla L(w) \neq 0$.}
\end{enumerate}
\end{exercise}
...
\label{ex:llc-regular-case}
    ...
    \hint{The volume of the ellipsoid $\{\boldsymbol{x} \in \R^d \mid \boldsymbol{x}^\top A \boldsymbol{x} < 1\}$ is proportional to $(\det A)^{-1/2}$.}
\end{exercise}
...
\begin{exercise}[Quadratic Valley Volume Scaling]
    ...
    \begin{enumerate}
        ...
        \hint{Use the volume of the ellipsoid from Exercise \ref{ex:llc-regular-case}}
    \end{enumerate}
\end{exercise}
...
\label{ex:lambda-upper-bound}
    ...
    \begin{enumerate}
        \item 
            ...
            \end{equation*}
            \hint{for any real symmetric matrix $H\in\R^{n\times n}$ and vector $v\in \R^n$, $v^\top H v \leq \Lambda  \|v\|^2$ where $\Lambda$ is the largest eigenvalue of $H$.}
...
% Disable hyperlinks in \cref and \nameref so that list entries link to the
% solutions rather than to the exercises named in the solution headings.
\let\oldcref\cref
\let\oldnameref\nameref
\renewcommand{\cref}[1]{\oldcref*{#1}}
\renewcommand{\nameref}[1]{\oldnameref*{#1}}
% Disable \section*{title}, including star
\renewcommand{\section}[2]{}
% Print the (patched) list of solutions
\listoftheorems[
    ignoreall,
    show={solthm},
    % swapnumber,
