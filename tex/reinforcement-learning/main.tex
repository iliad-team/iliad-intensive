...
\end{callout}
\begin{hint}
Condition on the first action and the first transition, then separate the immediate reward from the remaining return.
\end{hint}
\end{exercise}
...
\begin{remark}
This shows that the numerical policy evaluation algorithm from the ARENA materials converges to the correct solution.
\end{remark}
\end{exercise}
...
\begin{hint}
Show that $\mathcal{B}_{\pi'} V_\pi \ge V_\pi$ pointwise using \cref{prob:greedy_equals_B}, then iterate $\mathcal{B}_{\pi'}$ and take the limit.
\end{hint}
\end{exercise}
...
$$
\begin{remark}
This bound is not useful as a stopping criterion in practice as it depends on the unknown $V^*$.
\end{remark}
\end{exercise}
...
\begin{hint}
Write $V^* - V_n = \sum_{k=n}^{\infty}(V_{k+1} - V_k)$ (valid since $V_k \to V^*$), and bound each term using the contraction of $\mathcal{B}$.
\end{hint}
\end{exercise}
...
\begin{hint}
Telescope from $n$ as in \cref{prob:a_priori_bound}, but bound $\|V_{k+1} - V_k\|_\infty$ in terms of $\|V_n - V_{n-1}\|_\infty$ instead.
\end{hint}
\end{exercise}
...
\begin{hint}
Bound $\|V_n - V_{n-1}\|_\infty$ by repeated application of the contraction.
\end{hint}
\end{exercise}
...
$$
\begin{aligned}
\|V_n - V^*\|_\infty \;&=\; V^*(s) - V_n(s) \;=\; \frac{R_{\max}}{1-\gamma} - \frac{R_{\max}(1 - \gamma^n)}{1-\gamma} \\
\;&=\; \frac{\gamma^n}{1-\gamma}\, R_{\max},
\end{aligned}
$$
...
\begin{remark}
Combining \cref{prob:a_posteriori_bound,prob:a_priori_bound,prob:concrete_bound} gives a chain of progressively weaker but increasingly upfront-computable bounds:
\end{remark}
...
\begin{hint}
This is similar to the proof of \cref{prob:B_contraction}.
\end{hint}
\end{exercise}
...
$$
\begin{hint}
First show that $|w_t(s_t, a_t)| \le 2(R_{\max} + \gamma\, \|Q_t\|_\infty)$.
\end{hint}
\end{exercise}
...
\begin{remark}
The ARENA implementation uses a constant learning rate $\alpha$, which does not satisfy $\sum_t \alpha_t(s,a)^2 < \infty$. In practice, constant-rate Q-learning does not converge exactly but oscillates in a neighbourhood of $Q^*$ whose size shrinks with $\alpha$.
\end{remark}
\end{exercise}
...
\begin{hint}
Specialize the Bellman equation to a deterministic policy and write the resulting system of $|\cS|$ equations in matrix-vector form.
\end{hint}
\end{exercise}
...
\begin{remark}
The same derivation extends to stochastic policies by defining $\mathbf{P}^\pi_{s,s'} = \sum_{a} \pi(a \mid s)\, T(s' \mid s, a)$ and $\mathbf{r}^\pi_s = \sum_{a} \pi(a \mid s) \sum_{s'} T(s' \mid s, a)\, R(s, a, s')$. The matrix $\mathbf{P}^\pi$ remains row-stochastic, so the argument above applies unchanged.
\end{remark}
