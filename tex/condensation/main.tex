...
\Cref{sec:existence} is a proved example, not an
...
the trade-off when reconstruction is allowed to be inexact.
\end{solution}
...
information terms using their entropy definitions:
\begin{align*}
 &I(P;Q\mid M)-I(P;Q\mid U,M)\\
 &\quad=H(P\mid M)+H(Q\mid M)-H(P,Q\mid M)\\
 &\qquad-H(P\mid U,M)-H(Q\mid U,M)+H(P,Q\mid U,M)\\
 &\quad=I(U;P\mid M)+I(U;Q\mid M)-I(U;P,Q\mid M)\\
 &\quad=H(U\mid M)-H(U\mid P,M)-H(U\mid Q,M)\\
 &\qquad+H(U\mid P,Q,M).
\end{align*}
...
\begin{enumerate}
\item The first two families are listed in \cref{ex:bits}, and
\[
...
$\mathcal{G}_j=\bigcap_{\ell=1}^{j}\Up\{i_\ell\}$.
Every $\Up\{i\}$ is upward closed, as is every $\mathcal{G}_j$ by \cref{ex:intersections}.
We prove
\begin{equation}
 H(Y_{\Up A}\mid Z_{\mathcal{G}_j})\le
 \sum_{\ell=1}^{j}H(Y_{\Up A}\mid X_{i_\ell})+(j-1)\eta_Z
...
For $j=1$, $\mathcal{G}_1=\Up\{i_1\}$, and \cref{ex:transfer} gives
...
Apply \cref{ex:intersections} to the upward-closed families $\mathcal{G}_{j-1}$ and
...
\begin{align*}
 ...
 &=\sum_{\ell=1}^{j}H(Y_{\Up A}\mid X_{i_\ell})+(j-1)\eta_Z.
\end{align*}
The second line uses the induction hypothesis and \cref{ex:transfer}. This completes the
induction. At $j=k$, \cref{ex:intersections} gives $\mathcal{G}_k=\Up A$, hence
\[
 H(Y_{\Up A}\mid Z_{\Up A})
...
The proof uses LVM in $Z$ for \cref{ex:transfer}, the sender's reconstruction to bound those errors, and the receiver's Markov condition
...
$X_B$: for every $i\in B$ it contains $Z_{\Up\{i\}}$. \Cref{ex:transfer} therefore
...
families. Exactly the induction in \cref{ex:induction}, now with these families as leaves,
gives
\[
 H(U\mid Z_{\mathcal{R}})\le
...
\]
\item In \cref{ex:duplicates}, the independent model $Y$ has simple score $1+1+1=3$ and
