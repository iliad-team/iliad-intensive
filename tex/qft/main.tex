...
\begin{enumerate}
  ...
  classical learning theory. What is lost, and what is gained? E.g. for alignment, does one want worst-case guarantees or forecasting of typical capabilities?
...
\subsection{A note added by the demo pull request}

This subsection has no counterpart on the live site. It exists only to show
how the preview's diff view presents a newly added section: a heading and a
paragraph that appear on the right with a blank on the left.
...
\subsection{Why a loss is a negative log-likelihood}
...
%
\begin{equation}
    \prod_{\mu=1}^n p(y^\mu\,|\,x^\mu,\theta) \;\propto\; e^{-\beta\,\CL(\theta)}\,,\qquad \CL(\theta) = \frac{1}{2}\sum_{\mu=1}^n\big(f_\theta(x^\mu)-y^\mu\big)^2\,,\quad \beta = \sigma^{-2}\,. \label{mse-likelihood}
\end{equation}
%
...
\begin{enumerate}
  ...
  \item Show that the second cumulant is unchanged when $\CO$ is shifted by a constant, $\CO \to \CO + c$.
\end{enumerate}
\end{exercise}
...
%
\begin{equation}
    m(s) = \frac{1}{N}\sum_{i=1}^N s_i \;\in\; \{-1,\, -1 + \tfrac{2}{N},\, \dots,\, 1\}\,. \label{magnetization}
\end{equation}
%
...
\begin{enumerate}
    \item Differentiating under the integral sign, $\pd_J\log Z = \frac{1}{Z}\int d\theta\,\CO\,e^{-\beta E+J\CO} = \E_J(\CO)$, which at $J=0$ is $\E(\CO)$; since $F=-\log Z$, this is $-\pd_JF|_0$. Once more, $\pd_J^2\log Z = \E_J(\CO^2)-\E_J(\CO)^2 = \mathrm{Var}_J(\CO)$.
    ...
    \item Shifting $\CO \to \CO + c$ multiplies $Z(J)$ by $e^{Jc}$, which adds the linear term $Jc$ to $\log Z$; the second derivative in $J$ does not see it.
\end{enumerate}
\end{solution}
