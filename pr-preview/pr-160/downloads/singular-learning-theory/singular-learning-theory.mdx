...
- [1. Preliminaries](#1-preliminaries)
  - [1.1 Neural networks, parameters, and parameter–function maps](#11-neural-networks-parameters-and-parameterfunction-maps)
...
*Singular learning theory (SLT)* is a theory of learning that accounts for *degeneracy* in neural networks. What exactly is degeneracy? Let us first appeal to the Cambridge dictionary, which documents some features of typical uses of the word within mathematics:
...
This dictionary sense already captures the intuition we will make precise: a degenerate object sits at the boundary of a family, where something that is usually nonzero happens to vanish.
...
### 1.1 Neural networks, parameters, and parameter–function maps
...
1. A "parameter" is an individual dimension of parameter space (e.g., "initialise this parameter to the value $0.0$," or "this neural network has billions of parameters").
...
2. The **rectified linear unit (ReLU)** function $\relu(z) = \max(z, 0)$.
...
**Exercise 1.1 (Biased neurons).** A biased neuron is a neuron with an additional parameter that is added to the weighted sum of its inputs before it produces its output. Bias parameters allow each layer to represent an *affine* transform, rather than just a linear transform. For each of [Examples 1.1](#eg-neuron), [1.2](#eg-linear), [1.3](#eg-dln) and [1.4](#eg-mlp), extend the example to use biased neurons throughout. Define precisely the parameter space and the parameter–function map in each case, and count the parameters.
</Exercise>

<Solution>

For each architecture, we append a bias vector to every layer's output, before the next transformation (or activation function) is applied; the bias is a free parameter like any weight.
...
$$
f_{w,b}(x) = w^{\top} x + b, \qquad b \in \R.
$$
...
What kind of ordinary one-variable derivative does this reduce to, and why?
...
Define the curve in parameter space traced by the continuous symmetry, $\gamma : \R \to \W$ with $\gamma(t) = T_{t}(w)$. Fix $t_{0} \in \R$. This is the only step where continuity of the symmetry is used. Since $T_{t}$ is a continuous symmetry, the derivative along the curve at $t_{0}$ vanishes:

$$
\begin{aligned}D_{\gamma'(t_0)}\Phi(\gamma(t_{0}))&\propto \gamma'(t_{0}) \cdot \nabla \Phi(\gamma(t_{0})) \\&= \left.\frac{d}{dt}\right|_{t=t_0}\Phi(\gamma(t)) \quad\text{(chain rule for $\Phi \circ \gamma$)}\\&= \left.\frac{d}{dt}\right|_{t=t_0}\Phi(w) \quad\text{(symmetry, $\Phi \circ T_{t} = \Phi$)}\\&= 0. \quad\text{($\Phi(w)$ is constant in $t$)}\end{aligned}
$$
...
$$
F_{n}(\U) = nL_{n}(w^{*}) + \lambda_{\U}\log n - (\mu_{\U}- 1)\log \log n + O_{p}(1), \qquad n \to \infty.
$$
