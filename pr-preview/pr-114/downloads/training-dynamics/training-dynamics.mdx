...
**Exercise 3.1 (Gradient-flow equations).** For a two-layer DLN ($L=2$), the loss is given by $\mathcal{L}= \frac{1}{2}\|M - W_{2} W_{1}\|_{F}^{2}$. Assume that each weight matrix $W_{i}$ is diagonal. Derive the gradient flow equations for each layer. In particular, show that:
...
**Exercise 3.4 ($W_{1}^{\top}W_{1}$ in terms of $W$).** We now need to express $W_{1}^{\top} W_{1}$ and $W_{2} W_{2}^{\top}$ in terms of $W = W_{2} W_{1}$. Show that:
...
Analogous to [Exercise 3.4](#ex-td-2cii).
...
**Exercise 4.4 (Incremental learning).** Consider a teacher with $r$ nonzero singular values and a small uniform initialization $w_{\alpha}(0) = w_{0} \ll s_{r}$ for all $\alpha$. Qualitatively, discuss:
...
- [A Convergence Analysis of Gradient Descent for Deep Linear Neural Networks](https://arxiv.org/abs/1810.02281) Sanjeev Arora et al. — Convergence analysis of gradient descent
