...
\begin{align*}
    ...
    \quad \text{(chain rule, \cref{prob:chain_rule})} \\[4pt]
...
\begin{align*}
    V_\xi^{*,m}(\aes_{<t})
    ~=~ (1-\gamma) &\max_{a_t} \sum_{e_t} \xi(e_t \mid \aes_{<t} a_t) \\
    &\max_{a_{t+1}} \sum_{e_{t+1}} \xi(e_{t+1} \mid \aes_{<t+1} a_{t+1}) \cdots \\
...
\begin{remark}
With the Solomonoff prior, $w_\mu = 2^{-K(\mu)}$ may be astronomically small. The result guarantees non-zero value, not near-optimal value.
\end{remark}
...
\begin{remark}
We do not need to know which policy $\tilde\pi$ is self-optimizing, or whether it is computable. Mere existence suffices.
