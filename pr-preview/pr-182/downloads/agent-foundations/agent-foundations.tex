...
\begin{itemize}
\item
  \textbf{\Cref{ex:godel}: Gödel's second incompleteness theorem} (difficulty 4/5, importance 4/5). Via a self-referential program: a Gödel sentence is true, a consistent system cannot prove its own consistency, and this is exactly the Löbian obstacle to self-trust. Parts (a) true Gödel sentence, (b) no self-consistency proof, (c) the obstacle.
\item
  \textbf{\Cref{ex:lob}: Löb's theorem} (difficulty 3/5, importance 5/5). The three provability properties (necessitation, distribution, the Löb condition), a full proof of the theorem, and an application: FairBot programs cooperate by Löb's theorem. Parts (a-b) properties, (c-d) the proof, (e) FairBot.
\end{itemize}
...
$P$ from $\Box P$? L\"ob's theorem (\cref{ex:lob}) shows that the answer is no: any
consistent $L$ that derives $P$ from $\Box P$ for all $P$ is in fact inconsistent.

\begin{exercise}[Gödel's second incompleteness theorem]
\label{ex:godel}
\noindent
...
\begin{enumerate}
\item \label{ex:godel-a} Show that $G$ is true, assuming $L$ is consistent.

\begin{hint}

Consider two cases. Either $Z(Z)$ halts or it doesn't. In each case, use
...
\item \label{ex:godel-b} Show that if $L$ can prove its own consistency (i.e.\
$L \vdash \neg\Box\bot$), then $L$ is in fact inconsistent.

\begin{hint}

In \cref{ex:godel-a}, you argued from outside $L$ that $G$ is true, and the
...
\item \label{ex:godel-c} Suppose $L$ can vouch for all of its own proofs, meaning
...
\begin{itemize}
  \item[(i)] First, show that $L$ proves it never proves anything false. That is: for
    ...
    \begin{hint}
    The statement ``if $A$ then $B$'' is logically equivalent to ``if not
  ...
  \item[(ii)] Apply (i) with $P = \bot$, using the fact that $\neg\bot$ (``a
    ...
    $L \vdash \neg\Box\bot$, and use \cref{ex:godel-b} to finish.
\end{itemize}
\end{enumerate}

\begin{remark}[Self-trust, tiling agents, and the Löbian obstacle]
...
\medskip
\noindent
\Cref{ex:godel-c} shows this is impossible: any consistent system that asserts
...
\begin{enumerate}
\item $G$ is true (assuming $L$ consistent).
...
\soln The argument of \cref{ex:godel-a} used only the consistency of $L$, and each step is a finite manipulation of programs and proofs that $L$ can formalize. Reading it inside $L$: ``$Z(Z)$ halts'' yields $\bx G$ (by construction) and $\bx\Halts(Z(Z))$ (the bridge), hence $\bx K$; contrapositively $L\proves \neg\bx K \to G$. If $L\proves\neg\bx K$, modus ponens gives
\[
...
\item If $L\proves \bx P\to P$ for every $P$, then $L$ is inconsistent.
...
\emph{(ii)} Take $P=K$. Since $\neg K$ is a tautology, $L\proves\neg K$, so by (i) $L\proves\neg\bx K$. Thus $L$ proves its own consistency, and \cref{ex:godel-b} makes $L$ inconsistent. \qed
\end{enumerate}
\end{solution}

\begin{exercise}[Löb's theorem]
\label{ex:lob}
\noindent
...
\begin{enumerate}
\item \label{ex:lob-a} \textbf{Understanding Necessitation.} Necessitation says: if $\varphi$ is
provable in $L$, then $L$ can prove that $\varphi$ is provable. Explain why this is
true from the perspective of $\texttt{ProofSeeker}$.

\begin{hint}

If a proof of $\varphi$ exists, then $\texttt{ProofSeeker}(\varphi)$ will
find it and halt.
\end{hint}

\item \label{ex:lob-b} \textbf{Understanding Distribution.} It is helpful to think of a proof of
...
If both $\texttt{ProofSeeker}(\varphi \to \psi)$ and
$\texttt{ProofSeeker}(\varphi)$ halt, what can you do with their outputs? How can $L$ deduce that $\texttt{ProofSeeker}(\psi)$ halts?
\end{hint}

\item \label{ex:lob-c} We now prove L\"ob's theorem. The key ingredient is a self-referential sentence
constructed using the same idea as in \cref{ex:godel} (a sentence that talks about its
...
$G$ in \cref{ex:godel}; we take it as given here.)

Show that $L \vdash \Box\lambda \to \Box C$.

\begin{hint}

The L\"ob sentence says $L \vdash \lambda \to (\Box\lambda \to C)$.
...
\item \label{ex:lob-d} Now assume $L \vdash \Box C \to C$. Using \cref{ex:lob-c} and the
L\"ob sentence, derive $L \vdash C$.

\begin{hint}

From \cref{ex:lob-c}, you have $L \vdash \Box\lambda \to \Box C$. Chain this
with the assumption $L \vdash \Box C \to C$ to get
$L \vdash \Box\lambda \to C$. Now compare this with what $\lambda$ says about
itself.
\end{hint}

\begin{remark}[The Santa Claus paradox and what Löb adds beyond Gödel]
...
(\cref{ex:godel}) says that $L$ cannot prove its own consistency. This is already a
...
for tiling agents. Recall the setup from \cref{ex:godel}: a parent agent $A_1$ builds a
...
\item \label{ex:lob-e} In a one-shot Prisoner's Dilemma, two players each choose to
...
From FairBot's source code, $\Box A$ implies that
...
\begin{enumerate}
\item \textbf{Necessitation.} If $L\proves\varphi$ there is a concrete proof of $\varphi$; searching all strings, $\PS(\varphi)$ meets it and halts. ``$\PS(\varphi)$ halts'' is exactly $\bx\varphi$, and a halting run is finite, so by the bridge $L\proves\bx\varphi$. \qed

\item \textbf{Distribution.} A proof of $\varphi\to\psi$ turns a proof of $\varphi$ into one of $\psi$ (concatenate the two proofs and apply modus ponens). So if $\PS(\varphi\to\psi)$ and $\PS(\varphi)$ both halt, their outputs combine into a proof of $\psi$, whence $\PS(\psi)$ halts. This combining is a finite procedure $L$ can carry out, so $L\proves\bx(\varphi\to\psi)\to(\bx\varphi\to\bx\psi)$. \qed

\item The L\"ob sentence $\lambda$ satisfies $L\proves\lambda\leftrightarrow(\bx\lambda\to C)$.
...
\item Assuming $L\proves\bx C\to C$, derive $L\proves C$.

\soln Chaining $L\proves\bx\lambda\to\bx C$ (\cref{ex:lob-c}) with $L\proves\bx C\to C$ gives $L\proves\bx\lambda\to C$. The L\"ob equivalence gives the converse direction $L\proves(\bx\lambda\to C)\to\lambda$, so $L\proves\lambda$. By (N), $L\proves\bx\lambda$; with $L\proves\bx\lambda\to C$, modus ponens yields $L\proves C$. \qed

\item \textbf{FairBot.} With $A:=$ ``$\FB_1(\FB_2)=C$'' and $B:=$ ``$\FB_2(\FB_1)=C$'', show $L\proves A\wedge B$.
...
\begin{example}
You are deciding whether to carry an umbrella ($a_1$) or not
...
Every admissible decision rule is
...
\begin{enumerate}
\item \label{ex:complete-class-a} Show the following two facts:
\begin{itemize}
  \item[(i)] Any admissible decision rule $\delta = \sum_j \lambda_j\, a_j$ places
    ...
    \begin{hint}
    If $\delta$ places positive weight on a dominated pure action $a_j$,
  ...
  \item[(ii)] Every Bayes-optimal rule under a prior $\pi$ with $\pi_i > 0$ for all
    $i$ is admissible.
    \begin{hint}
    If some $\delta'$ dominated $\delta$, compare their expected
    utilities. What does $\pi_i > 0$ ensure?
    \end{hint}
\end{itemize}

\item \label{ex:complete-class-b} A \emph{face} $F$ of the Pareto frontier is a maximal convex
...
\item \label{ex:complete-class-c} Let $\pi$ be a vector perpendicular to the subspace $H$ from
\cref{ex:complete-class-b}, normalized so that $\pi_i > 0$ for all $i$ and $\sum_i \pi_i = 1$. You may
...
\begin{enumerate}
\item \emph{(i)} An admissible $\delta=\sum_j\lambda_j a_j$ puts $\lambda_j=0$ on every non-Pareto-optimal (dominated) $a_j$.
...
\emph{(ii)} Every Bayes-optimal rule under a prior $\pi$ with $\pi_i>0$ for all $i$ is admissible.
...
\item For a face $\mathcal F=\{\sum_{l=1}^p\mu_l\,r(a_{i_l}):\mu_l\ge0,\ \sum_l\mu_l=1\}$ with $v_l:=r(a_{i_l})-r(a_{i_1})$ and $H=\operatorname{span}\{v_2,\dots,v_p\}$: $H$ is exactly the set of directions tangent to $\mathcal F$.
...
\item Let $\pi\perp H$ be normalized so $\pi_i>0$ and $\sum_i\pi_i=1$, and assume no point of $R$ scores strictly higher under $\pi$ than the points of $\mathcal F$.

\soln \emph{(i)} For $x,x'\in\mathcal F$ we have $x-x'\in H$ by \cref{ex:complete-class-b}, so $\pi\cdot(x-x')=0$. Thus $\pi\cdot\rd$ equals a common value $c$ for all $\delta$ with $\rd\in\mathcal F$.
...
\emph{Conclusion.} If $\delta$ is admissible then $\rd\in F$ lies on some face $\mathcal F$; the prior $\pi$ built from $\mathcal F$ has all $\pi_i>0$ and, by (ii), makes $\delta$ Bayes-optimal. Conversely, by part (ii) of \cref{ex:complete-class-a} every Bayes-optimal rule under a strictly positive prior is admissible. The two classes coincide. \qed
\end{enumerate}
\end{solution}
...
Compute $D_{\mathrm{KL}}(P[X, A, O] \,\|\, P[X, A, O \mid do(A)])$
by expanding the log ratio using the factorizations above
\end{hint}

\begin{remark}[Maxwell's demon and the thermodynamics of optimization]
...
\begin{enumerate}
\item \label{ex:channel-additivity-a} Show that for any input distribution $P[X]$,
\[
  \mathrm{MI}(X;\, Y) \;=\;
...
Write each mutual information as a KL divergence between the joint and
the product of marginals, expand the log ratios using the channel factorization, and
collect terms.
\end{hint}

\item \label{ex:channel-additivity-b} Given any input distribution $P[X_1, X_2]$, define
\[
...
\begin{enumerate}
\item $\MI(X;Y)=\MI(X_1;Y_1)+\MI(X_2;Y_2)-\MI(Y_1;Y_2)$.
...
\item For $Q[X_1,X_2]:=P[X_1]P[X_2]$, $\MI_Q(X;Y)\ge\MI_P(X;Y)$.
...
\]
hence $\MI_Q(Y_1;Y_2)=0$. Applying \cref{ex:channel-additivity-a} under each distribution,
\begin{align*}
...
\item A throughput-maximizing input with $X_1\perp X_2$ always exists.

\soln The input simplex is compact and $\MI(X;Y)$ continuous, so a maximizer $P^*$ exists. Let $Q^*:=P^*[X_1]P^*[X_2]$. By \cref{ex:channel-additivity-b}, $\MI_{Q^*}(X;Y)\ge\MI_{P^*}(X;Y)=\max$, so $Q^*$ is also a maximizer, and under $Q^*$ the inputs are independent. \qed
\end{enumerate}
\end{solution}
