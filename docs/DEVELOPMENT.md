...
tex/<slug>/main.tex            <- the ONLY content in git (+ biblo.bib, fig/,
                                   optional slides.tex)
content/days.yml               <- committed: the teaching-day roster
...
- `scripts/build-status.mjs` runs last and writes `content/status.json` for
  `/admin/status`: the committed `content/days.yml` roster joined with what this
  build actually produced (which worksheets exist, which decks compiled, which
  download files were staged). A worksheet joins its day through its own
  `day: B.4` frontmatter, never through the roster — one file to touch when you
  port, and nothing to remember afterwards. Roster *data* errors are fatal (a
  duplicate code, a `day:` no day owns, a bad `source:` value): the fix is one
  line and a status page that silently drops a day is worse than a red build.
