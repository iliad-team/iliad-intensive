...
- `scripts/` — build pipeline: `build-content.mjs` runs `tex2mdx/` (LaTeX→MDX converter) then `build-status.mjs` (the `/admin/status` data); `schedule.mjs` reads/validates `schedule.yaml` for both; `watch.mjs` rebuilds on change. `house-lint.mjs` (`./run.sh lint`) sweeps every sheet for house-style violations without building.
...
- [`website.md`](docs/website.md) — hosting/domains: DNS, Cloudflare, the Pages custom domain, base paths, caching.
